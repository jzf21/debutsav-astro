...
export const EventTimeline = (props) => {
  ...
  useEffect(() => {
    document
      .querySelectorAll(".timeline-event-left-side-content svg")
      .forEach((icon) => {
        icon.classList.add(...["text-2xl", "md:text-4xl", "lg:text-5xl"]);
      });

    document
      .querySelectorAll(".timeline-event-right-side-content svg")
      .forEach((icon) => {
        icon.classList.add(...["text-2xl", "md:text-4xl", "lg:text-5xl"]);
      });
  ...
  const positionClassName = isEven
    ? "timeline-event-right"
    : "timeline-event-left";

  return (
    <div
      className={`timeline-event-component relative wrap overflow-hidden h-full px-4 py-2 ${className} ${positionClassName}`}
    >
